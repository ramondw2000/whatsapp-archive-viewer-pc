...
class MediaLoadQueue {
  private queue: Map<string, MediaLoadRequest> = new Map();
  private loading: Set<string> = new Set();
  private maxConcurrent = 3;
  private staggerIndex = 0;
  private loadCallback: (req: MediaLoadRequest) => Promise<string> = async () => '';

  setLoadCallback(cb: (req: MediaLoadRequest) => Promise<string>) {
    this.loadCallback = cb;
  }

  add(chatId: string, filename: string, distance: number) {
    const key = `${chatId}:${filename}`;
    const priority = distance < 500 ? 'high' : 'low';
    
    this.queue.set(key, { chatId, filename, priority, distance });
    // Stagger queue processing to spread re-renders across frames
    const delay = this.staggerIndex * 30;
    this.staggerIndex++;
    setTimeout(() => {
      this.staggerIndex = Math.max(0, this.staggerIndex - 1);
      this.process();
    }, delay);
  }
  ...
  private async process() {
    ...
    for (const req of sorted) {
      ...
      const key = `${req.chatId}:${req.filename}`;
