...
import { LazyMediaImage, useLazyVisibility, setMediaFallback, setToastCallback, showModuleToast, mediaCache } from "./LazyMediaImage";
...
function LoadingOverlay({ message, detail }: { message: string; detail?: string }) {
...
function DeleteConfirmationDialog({ chatId: _chatId, chatName, onSave, onDelete, onCancel, hasChanges }: {
...
function ExportSuccessDialog({ message, onClose }: {
...
function SuccessNotificationDialog({ chatName, onClose }: {
  ...
  return (
    ...
    <div className="dialog-overlay">
      ...
      <div className="dialog-content">
        ...
        <h3 style={{ textAlign: "center", marginBottom: "12px" }}>Changes Saved</h3>


        <p style={{ textAlign: "center", color: "var(--text-secondary, #666)", marginBottom: "24px" }}>


          Changes kept for "{chatName}" and chat deleted.<br />


          Your modifications will be re-applied when you import this chat again.
...
function DeleteSuccessDialog({ chatName, onClose }: {
  ...
  return (
    ...
    <div className="dialog-overlay">
      ...
      <div className="dialog-content">
        ...
        <h3 style={{ textAlign: "center", marginBottom: "12px" }}>Chat Deleted</h3>


        <p style={{ textAlign: "center", color: "var(--text-secondary, #666)", marginBottom: "24px" }}>


          "{chatName}" has been permanently deleted.<br />


          All chat data has been removed.
...
function UsernameDialog({ onSubmit, onCancel, hasExistingName }: { onSubmit: (name: string) => void; onCancel: () => void; hasExistingName: boolean }) {
...
interface BackgroundHistoryEntry {
...
function parsePhoneNumber(fullNumber: string): { optionId: string; localNumber: string } {
...
function formatPhoneNumber(optionId: string, localNumber: string): string {
...
function getPlaceholder(optionId: string): string {
...
function validatePhoneInput(input: string): string {
...
function GroupAvatar({ participants, size = "normal" }: { participants: string[]; size?: "normal" | "large" | "dialog" }) {
...
function GroupParticipantsDialog({
...
function LinkConfirmModal({ url, onConfirm, onCancel }: { url: string; onConfirm: () => void; onCancel: () => void }) {
...
function JumpButton({ direction, onClick, visible }: { direction: 'up' | 'down'; onClick: () => void; visible: boolean }) {
...
function StickerImage({ chatId, filename }: { chatId: string; filename: string }) {
...
function GifPlayerInner({ chatId, filename, onToggleType, autoPlay = false }: {


  chatId: string; filename: string; onToggleType?: () => void; autoPlay?: boolean;
...
function GifPlayer(props: Parameters<typeof GifPlayerInner>[0] & { index?: number }) {
...
function VideoPlayerInner({ chatId, filename, className, controls = true, autoPlay = false, onToggleType }: {
...
function VideoPlayer(props: Parameters<typeof VideoPlayerInner>[0] & { index?: number }) {
...
function AudioBase64Inner({ chatId, filename, isVideo }: {
...
function ExpandableText({ content, highlight, renderFn, hideExpandButton }: {
...
function AudioBase64(props: Parameters<typeof AudioBase64Inner>[0]) {
...
function MediaFallback({ filename, className, chatId, existsInZip }: { filename: string; className?: string; chatId?: string; existsInZip?: boolean; onRestored?: () => void }) {
...
function MediaImage({ chatId, filename, alt, className, onError, onLoad }: {
...
function FileAttachment(props: Parameters<typeof FileAttachmentInner>[0]) {
  ...
  return (
    ...
    <div ref={ref} style={style} className={className}>
      ...
      {isVisible ? <FileAttachmentInner {...props} /> : (
        ...
        <div className="media-skeleton" style={{ width: '100%', minHeight: 60 }}><div className="skeleton-shimmer"></div></div>
...
function FileTypeBadge({ filename, tagExt }: { filename: string; tagExt?: string | null }) {
...
const GAME_EXTS = new Set(["gb","gbc","gba","nds","n64","z64","v64","sfc","smc","nes","gg","sms","md","gen","pce","nsp","xci","cia","3ds","rom"]);


const EXT_DESCRIPTIONS: Record<string, string> = {
...
function FileAttachmentInner({ msg, idx, chatId, onTagSaved, showToast }: {
  ...
  // Derive effective extension from display name first, then original filename
  ...
  const hasExt       = !!extOf(filename); // original file has a real extension
  ...
  function saveRename() {
    ...
    // Build new filename: preserve base name, apply new extension
    ...
    // Always sync tag_ext to match the new extension (or clear it if no ext in new name)
    ...
    // Rename actual file on disk if extension changed
    ...
    <div className="file-attachment-wrapper">
      ...
      <div className="media-content file-attachment">
        ...
        <span className="file-attachment-actions">
          ...
          {/* Info popup button — always shown, extra useful for game files */}


          <button className="file-tag-btn" title="File info" onClick={openInfo}>ℹ️</button>


          {/* Rename/tag button — for extensionless OR tagged/renamed files */}
      ...
      {/* vCard error message with retry */}
...
const IMAGE_EXTS = new Set(["jpg","jpeg","png","gif","webp","bmp","heic","heif","svg","tif","tiff","avif","jfif","ico"]);


const VIDEO_EXTS = new Set(["mp4","mov","avi","mkv","webm","m4v","ts","flv","wmv"]);


const AUDIO_EXTS = new Set(["mp3","ogg","opus","wav","m4a","aac","3gp","3gpp","amr","flac","wma"]);
...
function StartScreen({ onOpenChats }: { onOpenChats: () => void }) {
  ...
  const handleCheckUpdates = async () => {
    ...
    try {
      ...
      const repoName = "whatsapp-chat-backup-tool-2.0";
      ...
      const currentVersion = "0.1.0"; // Should match package.json version
...
function App() {
  ...
  setMediaFallback(MediaFallback);
  ...
  function renderMessageText(text: string, query?: string): React.ReactNode {
    ...
    while ((match = URL_RE.exec(text)) !== null) {
      ...
      parts.push(
        ...
        <a
          ...
          onClick={e => { e.preventDefault(); openLinkWithConfirm(url); }}
  ...
  async function handleGroupPhotoUpload() {
    ...
    try {
      ...
      showToast("Failed to upload group photo: " + err);
...
function MessageRenderer({
  ...
  setVcardContact?: React.Dispatch<React.SetStateAction<{ name: string | null; phones: string[]; emails: string[] } | null>>;
